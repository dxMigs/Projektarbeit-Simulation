...
position_rakete = (screen_center[0], screen_center[1] - float(300_000_000*VH))
velocity_rakete = (math.sqrt(float(G)*float(masse_saturn)/float(300_000_000*VH)), 0) 
radius_rakete = 4
...
def update_screen():
    ...
    spur_breite = 3
    for body in bodies:
        if len(body.spur) > 2 and not body.name == "titan":
            pg.draw.lines(screen, BLUE, False, body.spur,spur_breite)
        elif len(body.spur) > 2 and body.name == "titan":
            pg.draw.lines(screen, TITAN_FARBE, False, body.spur,spur_breite)
...
def transfer_beschleunigen(SP_line, SR_line, distance):
    ...
    #-1400000
    #needed_velocity = math.sqrt(float(G) * float(Saturn.mass) * ((2/(float(SR_line.length())))-(1/((SP_line.length()+90000*VH+float((distance)*VH)+float(SR_line.length()))/2))))
    needed_velocity = math.sqrt(float(G) * float(Saturn.mass) * ((2/(float(SR_line.length())))-(1/((SP_line.length()-1400000*VH+float((distance)*VH)+float(SR_line.length()))/2))))
    ...
    if not beschleunigt:
        ...
        final_speed = rocket_speed()
        print(inititial_velocity, final_speed)
        delta_v_requirement(inititial_velocity, final_speed)
        beschleunigt = True 
...
def show_info():
    ...
    draw_text(f"Saturn ΔD, Pa, Aa: {daten_saturn}", text_font, LIGHT_GREY, 5, 65)
...
while running:
    ...
    swing_by_acceleration('titan', 2566, 1.55, 1.55, 30)
    show_info()
    zeit_vergangen()
