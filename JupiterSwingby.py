...
class Body:
    ...
    def update_position(self):
        ...
        self.repetitions += 1
        if self.name == 'rakete' and self.repetitions >= 75000/dt:
            self.spur.append(self.pos.xy)
            self.repetitions = 0
        elif self.name != 'rakete' and self.repetitions >= 75000/dt: 
            self.spur.append(self.pos.xy)
            self.repetitions = 0
        #if not self.name == 'rakete' and len(self.spur) > 500/dt*25: self.spur.pop(0)
...
GREY = (130, 130, 130)
LIGHT_GREY = BLACK #(170, 170, 170)
YELLOW = (235,235,0)
JUPITER_FARBE = (200, 171, 148)
ORANGE = (255,165,0)
...
#Fonts
text_font = pg.font.SysFont("Arial", 21)
...
#Saturn Objekt
spawn_angle_saturn = -math.radians(81.5235)
...
def update_screen():
    ...
    for body in bodies:
        if len(body.spur) > 2:
            if body.name == "rakete":
                pg.draw.lines(screen, BLUE, False, body.spur,3)
            elif body.name == "jupiter":
                pg.draw.lines(screen, JUPITER_FARBE, False, body.spur,3)
            elif body.name == "saturn":
                pg.draw.lines(screen, SATURN_FARBE, False, body.spur,3)
            elif body.name == "erde":
                pg.draw.lines(screen, EARTH_COLOR, False, body.spur,3)
            elif body.name == "venus":
                pg.draw.lines(screen, ORANGE, False, body.spur,3)
...
def swing_by_acceleration(planet, distance, x_acceleration, y_acceleration, delay):
    ...
    if round(int(get_distance_periapsis(planet)/1000), 1) == distance and not beschleunigt_2:
        ...
        if counter >= delay:
            ...
            relativ_speed_x = Rakete.velocity[0] - Saturn.velocity[0]
            relativ_speed_y = Rakete.velocity[1] - Saturn.velocity[1]
            relativ_speed_x *= x_acceleration
            relativ_speed_y *= y_acceleration
            Rakete.velocity[0] = relativ_speed_x + Saturn.velocity[0]
            Rakete.velocity[1] = relativ_speed_y + Saturn.velocity[1]
...
def get_escape_velocity(distance, velocity) -> float:
    soi = int(48.3e6)
    if abs(int(distance/1000)-soi) <= 100:
...
axis_counter = 0
def get_axis():
    global axis_counter, speed_list, distances_list
    if axis_counter == 2:
        speed_list.append(int(rocket_speed()))
        distances_list.append(round(int(get_actual_distance(Sonne)/1000), 1))
        axis_counter = 0
    axis_counter += 1
...
while running:
    ...
    #swing_by_acceleration('jupiter', 2910, 2.5, 2.5, 30)
    show_info()
    zeit_vergangen()
    if round(int(get_actual_distance(Saturn)/1000), 1) == 300_014:
        relativ_vektor = Rakete.velocity-Saturn.velocity
        relativ_speed = math.sqrt((float(relativ_vektor[0]*UK))**2+(float(relativ_vektor[1]*UK))**2)
        print(relativ_speed)
    get_axis()
...
pg.quit()
x_data = numpy.array(distances_list)
y_data = numpy.array(speed_list)
plt.title("Geschwindigkeit der Sonde")
plt.xlabel("Entfernung zur Sonne in km")
